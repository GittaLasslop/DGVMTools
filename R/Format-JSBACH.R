...
############################################################################################################################
############################ FUNCTIONS TO HANDLE JSBACH FILES ###########################################################
############################################################################################################################


#' Get a Field for JSBACH
#' 
#' 
#' An internal function that reads data from an JSBACH .nc file.  
#' 
...
#' @param jsbach.veg.file The filename of the JSBACH veg .nc file as a character string
...
#' @keywords internal
getField_JSBACH <- function(source,
                           quant,
                           target.STAInfo,
                           verbose = FALSE,
                           jsbach.veg.file) {
  ...
  veg.file=file.path(source@dir,jsbach.veg.file)
  
  if(file.exists(veg.file)){ 
    if(verbose) message(paste("Found and opening file", veg.file, sep = " "))
    if(verbose) veg.nc <- ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE )
    else veg.nc <- invisible(ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE ))
  }
  else {
    stop("Can't find veg file on disk")
  }
  
  
  # get lons and lat 
  all.lats <- veg.nc$dim$lat$vals  
  all.lons <- veg.nc$dim$lon$vals 
  print(all.lats)
  print(all.lons)
  ...
  if(quant@id == "LAI_std") { # or other variables in veg file
    
    if(verbose) message(paste0("Opening file ", veg.file))     
    if(verbose) this.nc <- ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE )
    else this.nc <- invisible(ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE ))
    
    
  }
  
  else if(quant@id == "aGPP_std") { # or other variables in veg file
    
    if(verbose) message(paste0("Opening file ", veg.file))     
    if(verbose) this.nc <- ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE )
    else this.nc <- invisible(ncdf4::nc_open(veg.file, readunlim=FALSE, verbose=verbose, suppress_dimvals=FALSE ))
    
    this.slice <- ncdf4::ncvar_get(veg.nc, "box_GPP_yDayMean", start = c(1,1,1,1), count = c(-1,-1,-1,-1), verbose = verbose)
   
    # multiply by 365.25 and 24*60*60 and conversion factor for mols to kgs   
    factor.conv = 365.25 * 24 *60 * 60 * 12 / 1000
    this.slice = this.slice * factor.conv
    
    dimnames(this.slice) <- list(all.lons, all.lats, 1:11)
    
    print(dimnames(this.slice))
    
    # prepare data.table from the slice (array)
    for(counter in 1:dim(this.slice)[3]){
      
      print(dim(this.slice[,,counter]))
      
      
    }
    #this.slice.dt <- as.data.table(melt(this.slice))
    #this.slice.dt <- stats::na.omit(this.slice.dt)
    #setnames(this.slice.dt, "value", "aGPP_std")
  ...
  getGlobalAttribute <- function(attr.name, global.attributes) {
    dgvm.attr.name <- paste("JSBACH", attr.name, sep = "_")
    if(dgvm.attr.name %in% names(global.attributes)) {
      return(global.attributes[[dgvm.attr.name]]) 
    }
    else {
      warning(paste0("No ", attr.name,  " attribute (", dgvm.attr.name, " ) found in file ", file.name.nc, ".  This is not JSBACH compliant.  Right now setting attribute to NULL, but this might cause problems later"))
      return(NULL)
    }
  }
  ...
  if(file.exists(file.name.nc)){ 
    ...
    stop("Gunzipping not yet supported for JSBACH.")
  }
  else {
  ...
  if(!is.null(xmin) & !is.null(xmax) & !is.null(ymin) & !is.null(ymax)){
    ...
    if(verbose) message("Setting spatial extent from JSBACH attributes")
  }
    ...
    if(verbose) message(paste("No spatial extent determinable from JSBACH", file.name.nc,"file. A filler extent will be set."))
    warning(paste("No spatial extent determinable from JSBACH", file.name.nc,"file. A filler extent will be set."))
...
determineQuantities_JSBACH <- function(source){
...
#' Detemine PFTs present in an JSBACH source 
#' 
#' @param x  A Source objects describing a JSBACH source
...
determinePFTs_JSBACH <- function(x, variables) {
...
#' @description \code{JSBACH} - a format defined here to unify the multitude of different datasets into a common format.  It is essentially CF-compliant netCDF with a couplf of extra attributes defined. 
#' Can be produced using the companion JSBACH package. 
#' 
...
JSBACH <- new("Format",
             ...
             determinePFTs = determinePFTs_JSBACH,
             
             # FUNCTION TO LIST ALL QUANTIES AVAILABLE IN A RUN
             determineQuantities = determineQuantities_JSBACH,
             
             # FUNCTION TO READ A FIELD 
             getField = getField_JSBACH,
             
             # DEFAULT GLOBAL PFTS  
             default.pfts = list(),
             
             # QUANTITIES THAT CAN BE PULLED DIRECTLY FROM LPJ-GUESS RUNS  
             quantities = JSBACH.quantities
...
GUESS.PFTs <- list(
  ...
  TeBE = new("PFT",
             id = "ETET",
             name = "Extra-tropical Evergreen Tree",
             growth.form = "Tree",
             leaf.form = "None",
             phenology = "Evergreen",
             climate.zone = "Temperate/Boreal",
             colour = "darkgreen",
  ...
  TeNE = new("PFT",
             id = "ETDT",
             name = "Extra-tropical deciduous Tree",
             growth.form = "Tree",
             leaf.form = "None",
             phenology = "Deciduous",
             climate.zone = "Temperate/Boreal",
